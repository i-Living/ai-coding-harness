...
> **Non-Bun projects:** substitute — `prettier --write`/`ruff format` for ①, `npm test`/`pytest`/`cargo test` for ④, `npm run build`/`cargo build` for ③, `eslint`/`ruff check`/`clippy` for ②. The gate structure is stack-agnostic; only the commands change.
