...
metadata:
  hermes:
    tags: [define, interview, intent, requirements, pre-spec, factory-model]
    category: define
    related_skills: [spec-driven-development, planning-and-task-breakdown, idea-refine]
---

# Interview Me — Pre-Spec Intent Extraction
...
This skill closes the gap before it costs anything. It is the **Phase 0** before `spec-driven-development` — where you ask one question at a time, with your best guess attached, until you can predict what the user is going to say before they say it.
...
This skill needs a live, responsive user. **Do not invoke in non-interactive contexts** like CI pipelines, scheduled runs, cron jobs, or autonomous loops. If you're in one of those and the ask is underspecified, flag that as a blocker for the user instead of guessing.
...
On Hermes Agent, use the `clarify` tool for the one-question-at-a-time pattern:

```python
clarify(
    question="When you say 'how are we doing?', who's asking — you alone or the team?",
    choices=["Just me — personal tracker", "Engineering team — standup report", "Up the chain — exec/board summary"]
)
```

The `clarify` tool naturally enforces one-question-at-a-time with structured choices, fitting the interview pattern. Use `choices` when you have concrete alternatives; use open-ended `clarify` (no choices) for exploratory questions.
...
- **Don't batch questions** — one at a time. Three questions in one message = the user will answer only the last one.
- **Don't skip the guess** — a question without a hypothesis is a survey. The guess surfaces YOUR assumptions.
- **"Whatever you think is best" is NOT a yes** — it's delegation. Re-ask with two concrete choices.
- **Don't skip "Out of scope"** — half of misalignment is unstated disagreement about what's NOT being built.
- **The 95% test is checkable, not a vibe** — can you literally predict the next three answers? If not, keep going.
- **Floor exists** — if confidence isn't rising after several rounds, something foundational is missing. Say so, don't grind.
