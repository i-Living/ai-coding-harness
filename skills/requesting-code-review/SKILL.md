...
**eval-harness (active):** This skill performs a comprehensive pre-commit review.
Between review steps and after each fix, `eval-harness` fires per-action gates
(format + check on changed files). The two skills work together:
`eval-harness` = per-action verification; `requesting-code-review` = full pre-commit pipeline.
Load both for maximum harness coverage.

**factory-mode:** If the project lacks harness components (Biome, tests, CI, pre-commit),
run `[[factory-mode]]` first — it audits and configures everything this skill depends on.
