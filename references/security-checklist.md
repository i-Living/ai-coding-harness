...
Quick reference for web application security. Use alongside the `code-review-and-quality` skill (security axis) and `requesting-code-review` (pre-commit scan).
...
- [ ] Lockfile committed; CI installs with `bun install --frozen-lockfile` or `npm ci` (not `npm install`)
