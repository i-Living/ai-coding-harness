...
The depth behind these items lives in `code-review-and-quality` (the five-axis review).
...
### Ship-readiness
- [ ] Security implications reviewed for any untrusted input, auth, or data handling (see `code-review-and-quality` + `requesting-code-review`)
- [ ] Observability in place for new critical paths (logs, metrics, traces) (see `agent-observability`)
...
- **Per release**: the full checklist is the floor; CI/CD adds the deploy-specific gates on top.
