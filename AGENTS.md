...
Factory Model harness for AI-assisted development. Thirteen skills that automate quality gates, project infrastructure, optimization, and the full discover→interview→spec→verify→improve pipeline.
...
| Skill | When to use |
|-------|-------------|
| **toolchain-discovery** | factory-mode Phase 1. Detects project stack → produces canonical command map. Never assume `bun` or `biome`. |
...
All skills use the project's **toolchain map** (generated by `toolchain-discovery`) as the source of truth for commands. Example for a Bun+Biome project:
```bash
bun run check    # = typecheck + lint + test
bun run format   # = auto-format via Biome
bun run build    # = production build
```
For non-Bun projects, the toolchain map resolves the equivalents (`pytest`, `cargo test`, `go test`, `ruff`, `eslint`, etc.).

### Stack assumptions
- **toolchain-discovery** runs first — no assumptions about package manager or formatter
- Skills reference the toolchain map instead of hardcoding commands
- Non-Bun projects are first-class: `pytest`, `cargo test`, `go test` are in the command map
...
- **toolchain-discovery** — load during factory-mode Phase 1. Produces command map consumed by all other skills.
